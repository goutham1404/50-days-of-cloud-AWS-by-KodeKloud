...
----------------------
```sh
aws ec2 describe-volumes --region us-east-1 --filters "Name=tag:Name,Values=datacenter-vol" --output table       # gets volume-id

aws ec2 create-snapshot --volume-id vol-0b18fed834b51f807 --description "datacenter Snapshot" --tag-specifications "ResourceType=snapshot,Tags=[{Key=Name,Value=datadenter-vol-ss}]" --region us-east-1 --output table            #  creates snapshot

aws ec2 wait snapshot-completed --snapshot-id snap-05a160903a8510cea       #  waits until snapshot created

aws ec2 describe-snapshots --snapshot-ids snap-05a160903a8510cea --region us-east-1 --output table     # verify snapshot created or not
```
