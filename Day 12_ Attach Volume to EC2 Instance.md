...
----------------------
```sh
aws ec2 describe-instances --region us-east-1 --filters "Name=tag:Name,Values=xfusion-ec2" --query "Reservations[].Instances[].InstanceId"   --output table        # Get Instance ID

aws ec2 describe-volumes --region us-east-1 --filters "Name=tag:Name,Values=xfusion-volume" --query "Volumes[].VolumeId" --output table       # Get Volume ID

aws ec2 attach-volume  --region us-east-1  --volume-id $VOLUME_ID --instance-id $INSTANCE_ID --device /dev/sdb         # Attach Volume
```
