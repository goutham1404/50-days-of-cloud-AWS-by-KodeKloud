...
----------------------
```sh
aws ec2 describe-images   --region us-east-1   --owners amazon   --filters "Name=name,Values=amzn2-ami-hvm-*-x86_64-gp2"  # to get ami id from aws_ami

aws ec2 create-key-pair   --key-name devops-kp   --query "KeyMaterial"   --output text > devops-kp.pem   # to create key pair and get pem file to present working directory

aws ec2 run-instances --image-id ami-0156001f0548e90b1 --instance-type t2.micro --key-name devops-kp --tag-specifications "ResourceType=instance,Tags=[{Key=Name,Value=devops-ec2}]"   # to create ec2 instance

aws ec2 describe-instances --instance-id i-036ccb060d4155bed  # to get the details of instance --public-ip

chmod 400 devops-kp.pem   # to secure the file only to root 

ssh -i devops-kp.pem ec2-user@98.81.140.37   # to access the server through pem file in your local
```
