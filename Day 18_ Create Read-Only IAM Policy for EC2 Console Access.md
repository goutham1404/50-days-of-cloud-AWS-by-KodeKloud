...
----------------------
```sh
vi ec2-readonly-policy.json              #  Create IAM policy JSON file
```
...
```sh
aws iam create-policy  --policy-name iampolicy_anita  --policy-document file://ec2-readonly-policy.json --region us-east-1           #   Create IAM policy with read-only EC2 permissions

aws iam list-policies  --query "Policies[?PolicyName=='iampolicy_anita'].PolicyName"  --output table           #  Verify policy exists
```
